...
Packet = {
  ...
  ConfCreateResponse = {
    ...
    parse = function(data)
      ...
      local responseLen, responseResult, domainParams, userdata
      responseLen, pos = decoder.decodeLength(data, pos)

      responseResult, pos = decoder:decode(data, pos)
      ccr.result = CONNECT_RESPONSE_RESULT[responseResult]
      ...
      domain_params, pos =  decoder:decode(data, pos)
      ...
      local _, userdata_pos = asn1.ASN1Decoder.decodeLength(userdata, 22 )
      while userdata:len() > userdata_pos do
        block_type, block_len  = string.unpack("<I2I2", userdata, userdata_pos)
        if block_type == 0x0c01 then
          -- 2.2.1.42 Server Core Data - TS_UD_SC_CORE
          proto_ver = string.unpack("<I4",userdata, userdata_pos + 4)
        ...
        elseif block_type == 0x0c02 then
          ...
          ccr.enc_level = string.unpack("B", userdata, userdata_pos + 8)
          ccr.enc_cipher= string.unpack("B", userdata, userdata_pos + 4)
        end
        userdata_pos = userdata_pos + block_len
      end
      return ccr
    end,
  },
...
Request = {
  ...
  MCSConnectInitial = {
    ...
    __tostring = function(self)
      ...
      "45 00 4d 00 50 00 2d 00 4c 00 41 00 50 00 2d 00 30 00 30 00 31 00 34 00 00 00 00 00 00 00 00 00" .. -- TS_UD_CS_CORE::clientName = EMP-LAP-0014
      ...
      "10 00" .. -- TS_UD_CS_CORE::highColorDepth = 16 bpp
      "08 00" .. -- TS_UD_CS_CORE::supportedColorDepths = 32 bpp
  ...
  MCSConnectInitialSSL = {
    ...
    __tostring = function(self)
      ...
      "00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 " ..
      "00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 " ..
      "00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 " ..  -- TS_UD_CS_CORE::clientDigProductId = "69712-783-0357974-42714"
      "00" .. -- TS_UD_CS_CORE::connectionType = 0 (not used as RNS_UD_CS_VALID_CONNECTION_TYPE not set)
      "00")   -- TS_UD_CS_CORE::pad1octet
      -- TS_UD_CS_CORE::serverSelectedProtocol
      .. string.pack("<I4", self.server_proto or 0) .. stdnse.fromhex(
